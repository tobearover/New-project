...
- 账号系统：注册 / 登录 / 退出，scrypt 密码哈希 + 随机 token 会话（7 天），生词本、复习与历史记录按账号隔离
- 单词学习：发音（英/美）、词性释义、例句、同反义词、派生词、考点解析、真题示例、记忆技巧
- 题目单词智能识别：拍照 / 相册导入，阿里云印刷体 OCR（识别成功显示「阿里云OCR」），自动提取单词并按考纲筛选、按重要程度分类；识别失败展示可读原因并可重试，不静默回退
- 识别历史与去重：每次识别自动记录且永久保留（仅用户手动删除才移除）；记录固化「词级结果快照」，历史详情与当初一致，不受日后词库/考纲变化影响；30 分钟窗口内重复内容自动提示（可强制重识别），提示不删除存储
- 识别结果筛选：顶部可伸缩设置条按词类（高频/常考/重点/认知）与词组显示开关过滤结果，选择本地持久化
- 识别结果展示：分类标题可点击折叠/展开整组（高频/常考默认展开、重点/认知默认收起，状态本地持久化）；点击单词跳转详情页，行内小按钮不触发跳转；详情页可返回识别结果
- 历史记录页：统一记录识别 / 测验 / 生词本操作，时间倒序分页加载，支持单条删除与批量删除（均二次确认）
- 单词手风琴：单词列表高频 / 重点词高亮标记，点击展开释义、例句、搭配，同一时间只展开一条
- 测验发音：拼写题提供美音 / 英音切换与播放发音按钮；真题模拟测验已按要求移除
- 详情页发音：例句可整句朗读或逐词点击朗读，搭配 / 词组一键朗读（Web Speech API）
- 结果自动过滤：未收录 / 超出所选考纲的词汇自动过滤，仅展示考纲内已收录词，保证识别结果准确
- 个性化生词本：加入生词本 / 已掌握 / 收藏，遗忘曲线复习提醒，间隔重复
- 智能测验：拼写（带发音）、词义匹配、听力辨词

## 技术栈

- 前端：React 18 + Vite + Tailwind CSS + React Router（PWA 支持）
- 后端：Node.js + Express + multer；OCR 走阿里云文字识别（RPC 签名直连，无第三方依赖）
- 数据：JSON 文件存储（`server/data/db.json`，首次启动自动由种子数据生成）

## 实现状态（MVP）

需求文档中的核心链路均已落地，种子词库规模为 11 个考纲 / 2.4 万+ 单词 / 16 个词组：

| 需求模块 | 实现情况 |
| --- | --- |
| 考纲范围选择 | ✅ 11 类考纲 + 版本、简介、适用人群、词库统计 |
| 考纲单词体系 | ✅ 高频/常考/重点/认知四级分类，发音、释义、例句、同反义、派生、词组 |
| 题目单词智能识别 | ✅ 拍照（getUserMedia）/ 相册导入 / 粘贴文本；客户端预处理（灰度/对比度/去噪）+ 阿里云 OCR + 考纲匹配分类 |
...
- 每次识别（文本/图片）都会记录到 `history`，摘要含引擎、考纲、匹配词与原文
- 历史条目可点击查看该次完整识别结果：`GET /api/recognition/history/:id` 优先读取入库时的
  **词级快照**（词 id + level + 释义 + 词组），结果与当初一致，不受日后词库/考纲变化影响；
  老记录（升级前无快照）自动回退为「原文 + 当前词库」重新提取，不丢数据
...
- 保留策略：识别历史**永久保留**，任何自动清理（窗口删除/条数上限/重启仅留 7 天）均已移除，
  只有用户手动删除（单条/批量/清空，二次确认）才移除；原文存储截断至 2000 字符，
  历史条数或 db.json 超过阈值时服务端仅打 warning 预警，不做自动删除

### 考纲词汇判断标准

识别结果仅展示**考纲内已收录词汇**：词条存在于合并词库（`words.js` 精编词条 +
`words_merged.js` 合并开源词库，运行合计 24,138 词，数据源为 english-vocabulary / dict-master / cet6-vocabulary），
且其 `exams` 包含所选考纲标识；未收录或不在考纲内的词自动过滤，不进入识别结果。

## 已知边界（后续迭代方向）

- 词库已由多开源数据源合并为 24,138 词并接入运行，后续可按考纲版本持续补充与人工审校（词条结构见 `server/src/seed/words.js`）
...
- 账号体系已上线（用户名+密码，scrypt 哈希，token 会话持久化在 db.json）；暂不支持邮箱/第三方登录、找回密码与多端会话管理
- 历史记录与生词本按 `userId` 隔离；接入登录前产生的旧本地数据归档在 `local` 桶中（未绑定任何账号，可后续提供迁移）
- 真题模拟测验已按需求移除；真题例句数据仍保留，用于单词详情页「真题示例」展示
- OCR 使用阿里云 RecognizeGeneral（`server/src/services/ocr.js` 为唯一接入点）；识别失败按
  网络 / 鉴权 / 服务 / 内容四级给出可读提示与重试，杜绝静默回退演示文本
...
> 说明：图片识别使用阿里云 OCR，需在服务器 `server/.env` 配置
> `ALIYUN_ACCESS_KEY_ID` / `ALIYUN_ACCESS_KEY_SECRET`
> （兼容 `ALIYUN_AK_ID` / `ALIYUN_AK_SECRET`、`ALIYUN_AK` / `ALIYUN_SK`；
> 可选 `ALIYUN_OCR_REGION` 默认 cn-hangzhou、`ALIYUN_OCR_ENDPOINT`、`ALIYUN_OCR_TIMEOUT_MS` 默认 10s）。
> 请求 10s 超时，网络抖动/5xx 自动重试一次；错误按网络/鉴权/服务/内容分级展示，不静默回退。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/auth/register` | 注册（成功自动登录，返回 token） |
...
| GET | `/api/history` | 统一历史列表（?type=recognition|quiz|wordbook&offset=&limit=，时间倒序） |
| GET | `/api/history/:id` | 历史详情（识别记录返回完整识别结果） |
| DELETE | `/api/history/:id` | 删除单条历史 |
| DELETE | `/api/history` | 批量删除（body: `{ ids: [...] }`） |

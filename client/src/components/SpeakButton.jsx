...
export default function SpeakButton({ word, text, accent = 'US', size = 'md', title }) {
  const { speaking, loading, speak } = useSpeech();
  ...
  return (
    <button
      ...
      disabled={loading}
      title={title || (loading ? '发音加载中…' : active ? '停止播放' : `播放发音（${accent}）`)}
      aria-label={title || `播放 ${content} 的${accent}发音`}
      className={`${sizeCls} inline-flex shrink-0 items-center justify-center rounded-full ring-1 transition disabled:opacity-60 ${
        loading
